...
This plugin version is 0.8.0, hashed as `8ee85f8a15c4b23f`.

Pass `version_hash` on every call to a Nestor MCP tool, like `{ "version_hash": "8ee85f8a15c4b23f", ... }`. The server compares this hash to the published release. It cannot be guessed or incremented, so never send another value than the one written here.
